...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/debussy_other_piano_pieces.git
...
* `Claude Debussy – Other Piano Pieces <https://github.com/DCMLab/debussy_other_piano_pieces/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by five files with identical names, each in its own folder. For example, the first piece, the *Étude retrouvée* (an alternative version of no. 11 of the *Douze Études*), has the following files:

-  ``MS3/l000_etude.mscx``: Uncompressed MuseScore 3.6.2 file including the music and annotation labels.
-  ``notes/l000_etude.notes.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/l000_etude.measures.tsv``: A table with relevant information about the measures in the score.
-  ``chords/l000_etude.chords.tsv``: A table containing layer-wise unique onset positions with the musical markup (such as dynamics, articulation, lyrics, figured bass, etc.).
-  ``harmonies/l000_etude.harmonies.tsv``: A table of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/l000_etude.tsv')
   notes = ms3.load_tsv('notes/l000_etude.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/debussy_other_piano_pieces/issues>`__ and feel free to fork and submit pull requests.
